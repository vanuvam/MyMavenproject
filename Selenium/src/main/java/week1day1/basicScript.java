...
//crms/fa link 
		...
		*/
		Select total = new Select(driver.findElementById("createLeadForm_currencyUomId"));
		List<WebElement> option1 = total.getOptions();
		
		int count=option1.size();
		
		Random rNum = new Random();
		rNum.nextInt(count-1);
		int countOpt = rNum.nextInt(count-1);
		System.out.println("The random value of currency: "+countOpt);
		total.selectByIndex(countOpt);
		
		System.out.println("The count of currencies: "+count);
		
		for (int i=0;i<=count-1;i++) {
			
			System.out.println(option1.get(i).getText() + " ----> "+ i);
			
		}
		
		/*System.out.println("The count of currencies: "+count);
		System.out.println("The first currency is list: ");*/
